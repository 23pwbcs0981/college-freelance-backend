...
exports.getClientData = async (req, res) => {
  ...
  try {
    ...
    const [projects] = await db.execute(
      "SELECT * FROM projects WHERE client_id = ? AND assigned_to IS NULL",
      [clientId]
    );
  ...
  WHERE a.project_id IN (${projectIds.map(() => "?").join(", ")})`;
    ...
    if (projectIds.length === 0) {
      ...
      const [applicationsData] = await db.execute(query, projectIds);
      applications = applicationsData.map((application) => {
        return {
          ...application,
          skills: JSON.parse(application.skills_required || "[]"), // Parse skills_required if available
        };
      });
    }
